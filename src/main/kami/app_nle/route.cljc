(ns kami.app-nle.route
  "The app's addressable views, and the fragment that addresses them.

  KAMI NLE is a **single-page app** (the kotoba-lang default — see the UI/UX
  section of the workspace CLAUDE.md and the `kotoba-uiux` skill). One document,
  one bundle, one mount; moving between the editor and the user-test dashboard
