(ns kami.app-nle.dashboard
  "The user-test dashboard, on the same **jp-go-dds** base as the editor.
...
(defn app []
  [:main
   (dds/container
    (dds/section {}
      (dds/heading 1 "KAMI NLE · User-test dashboard")
    ...
    (dds/section {:title "Import"}
      (dds/form-field
       {:label "Session artifacts" :for "sessions"
        :support "One or more .json files exported from the editor's user-test panel."}
       (dds/input-text
