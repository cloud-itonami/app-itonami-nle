(ns gen-page
  ...
  is now a single `->page` call against `kami.app-nle.theme`, so the page
...
(def dds-root
  ...
  first, because a temp worktree outside the superproject defeats every
  relative guess. Mirrors gftdcojp/itad's web/generate.cljs."
...
(def favicon
  ...
  The old hand-written page carried one of these as a data URI with `#38bdf8`
...
(defn page []
  (dds-page/->page
   {:title "KAMI NLE"
    :description "KAMI NLE — EDN-native non-linear video editor"
    :lang "ja"
    :css dds-css
    ;; DADS is a light design system; `:dark? true` is this library's own
    ;; inversion layer. A video editor grades against a dark surround.
    :dark? true
    :app-css theme/app-css*
    :head [favicon]}
   [:div {:id "app"} "KAMI NLE loading…"]
   [:script {:src "js/main.js"}]))

(defn dashboard-page []
  (dds-page/->page
   {:title "KAMI NLE · User-test dashboard"
    :description "Compare KAMI NLE user-test sessions exported by actor runs"
    ;; The copy on this page is English, as it was; the editor page says "ja".
    :lang "en"
    :css dds-css
    ;; Same appearance as the editor: two pages of one app, one surround.
    :dark? true
    :app-css theme/app-css*
    ...
    :head [favicon
           [:meta {:name "kotoba:app-shell" :content "kami-nle user-test dashboard"}]]}
   [:div {:id "app"} "Loading…"]
