...
(defn- spa-checks
  ...
  [(fn [] (p/let [scripts (.count (.locator page "script[src]"))
                  srcs (.allTextContents (.locator page "script[src]"))]
            (check! "the app ships one bundle" (= 1 scripts) (str scripts " " (pr-str (js->clj srcs))))))
   ...
   (fn [] (p/let [_ (.evaluate page "window.__spaWitness = 'alive'")
            ...
            (check! "crossing to the user-test view does not load a document"
                    (and (= "alive" (str witness))
                         (= "#/user-test" (str hash))
                         (= "KAMI NLE · User-test dashboard" (str t)))
                    (str witness " " hash " " t))))

   ;; And back — a one-way router is a link, not a router.
   (fn [] (p/let [_ (.click (.locator page "nav[aria-label='Views'] a:has-text('Editor')"))
                  _ (.waitForSelector page ".nle-track" #js {:timeout 15000})
                  witness (.evaluate page "window.__spaWitness ?? 'gone'")
                  tracks (.count (.locator page ".nle-track"))]
            (check! "and back to the editor, still without a document load"
                    (and (= "alive" (str witness)) (>= tracks 3))
                    (str witness " tracks=" tracks))))

   ;; The editor's own state has to outlive a view change; if it did not, the
   ;; single page would be buying nothing.
   (fn [] (p/let [_ (.click (.locator page ".nle-clip:has-text('Close up')"))
                  _ (settle page)
                  _ (.click (.locator page "nav[aria-label='Views'] a:has-text('User test')"))
                  _ (.waitForSelector page "#sessions" #js {:timeout 15000})
                  _ (.click (.locator page "nav[aria-label='Views'] a:has-text('Editor')"))
                  _ (.waitForSelector page ".nle-track" #js {:timeout 15000})
                  selected (.textContent (.first (.locator page ".nle-clip.selected")))]
            (check! "editor state survives the round trip"
                    (str/includes? (str selected) "Close up") (str selected))))])
...
(defn- dashboard-checks
  ...
  [(fn [] (p/let [t (.textContent (.locator page "main h1"))]
            (check! "user-test view mounted" (= "KAMI NLE · User-test dashboard" (str t)) t)))
   ...
   ;; `#sessions`, not `input[type=file]`: the editor has seven of those, and
   ...
   ;; live site, where it resolved to the editor's WebVTT import).
...
(defn -main []
  (p/let [browser (.launch (.-chromium pw) launch-opts)
          ...
          ;; spa-checks leaves us on the editor; cross once more for the view's
          ;; own checks.
          _ (.click (.locator page "nav[aria-label='Views'] a:has-text('User test')"))
